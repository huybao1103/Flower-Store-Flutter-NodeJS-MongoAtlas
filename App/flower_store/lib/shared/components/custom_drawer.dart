...
class _CustomSubmenuState extends State<CustomSubmenu> {
  ...
  Padding getMenuItemWithSubmenu(
      {required double width, required MenuItem menuItem}) {
    return Padding(
  ...
  SubmenuButton getMenus({required MenuItem menuItem, required double width}) {
...
InkWell commonSubMenuItem(
    {required String label, bool? first, void Function()? onTap}) {
  label = label.trim();
  String iconName =
